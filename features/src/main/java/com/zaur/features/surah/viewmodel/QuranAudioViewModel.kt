...
class QuranAudioViewModel(
    private val savedStateHandle: SavedStateHandle, private val quranAudioUseCase: QuranAudioUseCase
    ...
    suspend fun getRecitations(language: String) {
        val result = launchSafely { quranAudioUseCase.getRecitations(language) }
    ...
    suspend fun getChaptersAudioOfReciter(reciterId: Int, chapterNumber: Int) {
        val result =
            launchSafely { quranAudioUseCase.getChaptersAudioOfReciter(reciterId, chapterNumber) }
    ...
    suspend fun getVerseAudioFile(reciterId: Int, verseKey: String) {
        val result = launchSafely { quranAudioUseCase.getVerseAudioFile(reciterId, verseKey) }
